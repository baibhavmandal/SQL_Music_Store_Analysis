...
-- Write a query that returns one city that has highest sum of invoice
-- totals. Return both the city name & sum of all invoice totals.

select billing_city, sum(total) as _total from invoice
group by billing_city
order by _total desc

-- Q5: Who is the best customer? The customer who has spent the most
-- money wiil be declared the best customer. Write a query that returns
...
-- Q1: Write query to return the email , first name, last name, and genre
-- of all rocks music listeners. Return your list ordered alphabetically
...
-- Q2: Let's invite the artist who have written the most rock
...
-- Q1: Find how much amount spent by each customer artist?
-- Write a query to return customer, artist name and total spent.
...
select 
	...
	group by 1 -- hence group by artist_id
	order by 3 desc -- hence total_sales
	limit 1
)
select 
c.customer_id, 
